...
template <uint32_t D, uint32_t B>
ResultHeap IVFRN<D, B>::search_fast_scan(const Result* centroid_dist, float16_t* query_f16, float* rd_query, uint32_t nprobe, uint32_t k, float distK) const {
        ResultHeap KNNs;
...
ResultHeap IVFRN<D, B>::search(float* query, float* rd_query, uint32_t k, uint32_t nprobe, float distK) const{
    ...
    return search_fast_scan(centroid_dist, query_f16, rd_query, nprobe, k, distK);
#elif defined(SCAN)
    ResultHeap KNNs;
    free(query_f16);
